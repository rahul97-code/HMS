...
public class IPDOTExamEntry extends JDialog {
	...
	String exam_pagenumber = "0";
	  CopyOnWriteArrayList<String> charges_id = new CopyOnWriteArrayList<>();
	  CopyOnWriteArrayList<String> ward_time_min = new CopyOnWriteArrayList<>();
	  CopyOnWriteArrayList<String> ward_time_max = new CopyOnWriteArrayList<>();
	  CopyOnWriteArrayList<String> ot_charges = new CopyOnWriteArrayList<>();
	  CopyOnWriteArrayList<String> ot_id = new CopyOnWriteArrayList<>();
	  CopyOnWriteArrayList<String> ot_type = new CopyOnWriteArrayList<>();
	...
	private JTextField OTAmountTF;
	private JComboBox oTypeCB;
	private ChangeListener timeChangeListener;
	private JPanel panel_8;
	protected String toDateSTR="";
	protected OTType selectedOTType;
	private int ot_chargesTotal;
	...
	public static void main(String[] strings) {
		new IPDOTExamEntry(null).setVisible(true);
	}

	public IPDOTExamEntry(final LIS_System obj) {
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				IPDOTExamEntry.class.getResource("/icons/rotaryLogo.png")));
		setTitle("IPD New Exam Entry");
		setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		setResizable(false);
		setBounds(110, 50, 1136, 570);
		setModal(true);
		getAprovelAcess();
		...
		contentPane.setBorder(new TitledBorder(UIManager
				.getBorder("TitledBorder.border"), "New OT Entry Form",
		...
		searchBT.setIcon(new ImageIcon(IPDOTExamEntry.class
		...
		panel_2.setBounds(704, 242, 420, 211);
		...
		btnRemove.setBounds(283, 9, 112, 25);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(row==-1) {
		            JOptionPane.showMessageDialog(null, "Please Select First Row", "Input Error", JOptionPane.ERROR_MESSAGE);
		            return;
				}
				...
				System.out.println(Arrays.toString(examIdVector.toArray())+" code" +exam_nameid);
				...
				if (textField_2.getText().isEmpty() || Integer.parseInt(textField_2.getText())==0) {
		            JOptionPane.showMessageDialog(null, "Please enter a Amount value", "Input Error", JOptionPane.ERROR_MESSAGE);
		          return;
				}
				...
				examChargesVector.add(textField_2.getText().toString());
		...
		btnNewButton.setBounds(178, 11, 89, 23);
		panel_2.add(btnNewButton);
		
		textField_2 = new JTextField();
		textField_2.setBounds(59, 14, 95, 20);
		...
		textField_2.addKeyListener(new KeyAdapter() {
		    ...
		    public void keyTyped(KeyEvent e) {
		        char c = e.getKeyChar();
		        // If the character is not a digit or control key (backspace), we stop it
		        if (!Character.isDigit(c) && !Character.isISOControl(c)) {
		            e.consume(); // This prevents the character from being typed
		        }
		    }
		});
		
		JLabel lblNewLabel_3 = new JLabel("Amnt :");
		lblNewLabel_3.setBounds(12, 13, 52, 16);
...
//				String dataOt[] = new String[12];
//				dataOt[0] = "" + ipd_id;
//				dataOt[1] = "" + p_id;
//				dataOt[2] = "" + p_name;
//				dataOt[3] = DepartmentMain.departmentName;
//				dataOt[4] = toDateSTR;
//				dataOt[5] = "" +ConvertTime(fromTime.getFormatedTime().toString());
//				dataOt[6] = toDateSTR;
//				dataOt[7] =  "" +ConvertTime(toTime.getFormatedTime().toString());
//				dataOt[8] = DepartmentMain.userID;  //user id
//				dataOt[9] = DepartmentMain.userName;  //user name
//				dataOt[10] = selectedOTType.getOtId()+"";
//				dataOt[11] = selectedOTType.getOtType()+"";
//				int id = 0;
//				try {
//					System.out.println(Arrays.toString(dataOt));
//					id = db1.inserIPD_OT_Detail(dataOt);
//				} catch (Exception e1) {
//					// TODO Auto-generated catch block
//					e1.printStackTrace();
//				}
...
//				db1.closeConnection();
				JOptionPane.showMessageDialog(null, "OT Data saved successfully.","Success",JOptionPane.INFORMATION_MESSAGE);
//				try {
...
//				}
		...
		btnNewButton_3.setBounds(744, 465, 147, 44);
		...
		btnNewButton_4.setBounds(901, 465, 146, 44);
		contentPane.add(btnNewButton_4);
		btnNewButton_4.setIcon(new ImageIcon(IPDOTExamEntry.class
		...
		panel_3.setBorder(new TitledBorder(UIManager
				...
				TitledBorder.TOP, null, null));
		panel_3.setBounds(344, 242, 348, 211);
		...
		scrollPane_1.setBounds(358, 82, 401, 150);
		...
		panel_5.setBounds(358, 23, 201, 50);
		contentPane.add(panel_5);
		panel_5.setLayout(null);
		panel_5.setBorder(new TitledBorder(UIManager

				.getBorder("TitledBorder.border"), "OT Exam Detail :",
		...
		examNameCB.setBounds(6, 16, 189, 25);
		panel_5.add(examNameCB);
		JPanel panel_7 = new JPanel();
		panel_7.setBounds(571, 22, 188, 50);
		contentPane.add(panel_7);
		panel_7.setLayout(null);
		panel_7.setBorder(new TitledBorder(UIManager

				.getBorder("TitledBorder.border"), "OT Exam Search :",
		...
		textField_1.setBounds(21, 17, 144, 25);
		panel_7.add(textField_1);
		textField_1.setColumns(10);
		
		JPanel panel_4 = new JPanel();
		panel_4.setBounds(521, 467, 171, 50);
		...
		panel_8 = new JPanel();
		panel_8.setBounds(769, 24, 345, 211);
		...
		toDate.setBounds(129, 53, 204, 22);
		panel_8.add(toDate);
		...
		lblDate.setBounds(31, 55, 88, 18);
		panel_8.add(lblDate);
		
		 oTypeCB = new JComboBox();
		 oTypeCB.addActionListener(new ActionListener() {
			    @Override
			    public void actionPerformed(ActionEvent e) {
			         selectedOTType = (OTType) oTypeCB.getSelectedItem();
			        if (selectedOTType != null && !selectedOTType.getOtId().isEmpty()) {
			            System.out.println("Selected OT Type: " + selectedOTType.getOtType());
			            System.out.println("Selected OT ID: " + selectedOTType.getOtId());
			            int Range=calculateTimeInMinutes(fromTime,toTime);
			       
			            int maxTime=CalculateOTCharges(selectedOTType.toString(), Range);
				        OTAmountTF.setText(""+CalculateAmountOT(Range,maxTime,ot_chargesTotal));
			        } else {
			        	 toTime.setTime(new Date());
			        	 fromTime.setTime(new Date());
			        	 OTAmountTF.setText("0");
			            System.out.println("No OT Type selected");
			        }
			       
			       
			    }
			});
		oTypeCB.setBounds(126, 11, 207, 24);
		panel_8.add(oTypeCB);
		
		JLabel lblNewLabel = new JLabel("OT Type :");
		lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 11));
		lblNewLabel.setBounds(31, 11, 66, 24);
		panel_8.add(lblNewLabel);
		
		JLabel lblTime = new JLabel("Wheel Out :");
		lblTime.setFont(new Font("Tahoma", Font.PLAIN, 11));
		lblTime.setBounds(31, 133, 66, 22);
		panel_8.add(lblTime);
		
		toTime = new JTimeChooser();
		
		toTime.setBounds(130, 133, 170, 25);
		
		
		toTime.getTimeField().setFont(new Font("Tahoma", Font.BOLD, 13));
		toTime.setTime(new Date());
		toTime.getTimeField().addMouseWheelListener(new MouseWheelListener() {
				@Override
				public void mouseWheelMoved(MouseWheelEvent e) {
					...
					try {
					 ...
					 OTType selectedOTType = (OTType) oTypeCB.getSelectedItem();
				        if (selectedOTType == null || selectedOTType.getOtType().equals("Select OT")) {
								JOptionPane
								.showMessageDialog(
										null,
										"Please Select OT Type",
										"Input Error", JOptionPane.ERROR_MESSAGE);
		...
		toTime.getTimeField().addKeyListener(new KeyAdapter() {
            @Override
		...
		lblWheelIn.setFont(new Font("Tahoma", Font.PLAIN, 11));
		lblWheelIn.setBounds(31, 92, 66, 22);
		panel_8.add(lblWheelIn);
		fromTime = new JTimeChooser();
		
		fromTime.setBounds(130, 92, 170, 25);
		
		fromTime.getTimeField().setFont(new Font("Tahoma", Font.BOLD, 13));
		fromTime.setTime(new Date());
        // Handle mouse wheel scroll with MouseWheelListener
        fromTime.getTimeField().addMouseWheelListener(new MouseWheelListener() {
			@Override
			public void mouseWheelMoved(MouseWheelEvent e) {
				// TODO Auto-generated method stub
				 OTType selectedOTType = (OTType) oTypeCB.getSelectedItem();
        ...
        fromTime.getTimeField().addKeyListener(new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                	 OTType selectedOTType = (OTType) oTypeCB.getSelectedItem();
		...
		JLabel lblNewLabel_5 = new JLabel("Calculation :");
		lblNewLabel_5.setBounds(29, 182, 78, 14);
		lblNewLabel_5.setFont(new Font("Tahoma", Font.PLAIN, 11));
		panel_8.add(lblNewLabel_5);
		
		OTAmountTF = new JTextField("0");
		OTAmountTF.setEditable(false);
		OTAmountTF.setBounds(129, 179, 88, 20);
		panel_8.add(OTAmountTF);
		OTAmountTF.setColumns(10);
		
		JButton btnNewButton_1 = new JButton("Save Time");
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (oTypeCB.getSelectedIndex()==0) {
					JOptionPane.showMessageDialog(null, "Please select OT Type.",
							"Input Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				db1.closeConnection();
				JOptionPane.showMessageDialog(null, "OT Time saved successfully.","Success",JOptionPane.INFORMATION_MESSAGE);
			}
		});
		btnNewButton_1.setBounds(228, 177, 105, 23);
	...
	public void getAllExamList() {
		...
		ResultSet resultSet = dbConnection.retrieveAllOTExams(ExamMaterTableIndex);
		try {
			while (resultSet.next()) {
	...
	public void getAllOTType() {
		DefaultComboBoxModel<OTType> ottype = new DefaultComboBoxModel<>();
	    ottype.addElement(new OTType("Select OT", "")); // Add a default option
	    
	    ExamDBConnection dbConnection = new ExamDBConnection();
	    ResultSet resultSet = dbConnection.retrieveAllOTType();
	    
	    try {
	        while (resultSet.next()) {
	            String otType = resultSet.getObject(1).toString(); // Assuming your result set column names
	            String otId = resultSet.getObject(2).toString();
	            OTType ot = new OTType(otType, otId);
	            ottype.addElement(ot);
	        }
	    } catch (SQLException e) {
	        e.printStackTrace();
	    } finally {
	    ...
	    panel_8.add(toTime);
		panel_8.add(fromTime);
		 
	    oTypeCB.setModel(ottype);
	    oTypeCB.setSelectedIndex(0); // Select the first item (default "Select OT")
	}
	public void populateExamTable(String ExamCat) {

		try {
			ExamDBConnection dbConnection = new ExamDBConnection();
			ResultSet rs = dbConnection.GetAllOTExams(ExamMaterTableIndex,ExamCat,AprovelAccess);
...
//		exam_charge = "" + (int) charges;
	...
	public void getallotcharges() {
		try {
			...
			ot_type.clear();
			IPDDBConnection db = new IPDDBConnection();
			ResultSet rs = db.getallotcharges();
			while (rs.next()) {
				charges_id.add(rs.getObject(1).toString());
				ward_time_min.add(rs.getObject(2).toString());
				ward_time_max.add(rs.getObject(3).toString());
				ot_charges.add(rs.getObject(4).toString());
				ot_id.add(rs.getObject(5).toString());
				ot_type.add(rs.getObject(6).toString());
				
			}
			for (String item : charges_id) {
	...
	public int CalculateOTCharges(String OT_type, int Range) {
		int maxTime=0;
		ot_chargesTotal=0;
	    for (int i = 0; i < charges_id.size(); i++) {
	        // Check if the OT_type matches and if the Range is within the min/max time range
	        if (ot_type.get(i).equals(OT_type)) {
	            int minTime = Integer.parseInt(ward_time_min.get(i));  // Convert min time to integer
	             maxTime = Integer.parseInt(ward_time_max.get(i));  // Convert max time to integer
	            
	                ot_chargesTotal=Integer.parseInt(ot_charges.get(i));
	                System.out.println("Charge ID: " + ot_charges.get(i));
	                System.out.println("OT Charge: " + ot_charges.get(i));
	                System.out.println("OT ID: " + ot_id.get(i));
	                System.out.println("OT Type: " + ot_type.get(i));
	                System.out.println("Ward Time Min: " + ward_time_min.get(i));
	                System.out.println("Ward Time Max: " + ward_time_max.get(i));
	...
	public static int calculateTimeInMinutes(JTimeChooser fromTime, JTimeChooser toTime) {
        // Get the time from the TimeField component inside JTimeChooser
        String fromTimeText = fromTime.getTimeField().getText();
        String toTimeText = toTime.getTimeField().getText();

        // Define the time format (assuming it's in HH:mm:ss format)
        SimpleDateFormat timeFormat = new SimpleDateFormat("HH:mm:ss");

        try {
            // Parse the times into Date objects
            Date fromDate = timeFormat.parse(fromTimeText);
            Date toDate = timeFormat.parse(toTimeText);
            ...
            return (int) (differenceInMillis / 60000);  // Divide by 60,000 to get minutes
        } catch (ParseException e) {
            e.printStackTrace();
            return 0; // Return 0 if there's an error parsing the time
        }
    }
	...
	public double CalculateAmountOT(double Range, int MaxTime, double TotalAmount ) {
		double multitime=Range/15;
		multitime = (int) Math.ceil(multitime);
		int fixedSlab= 15;
		double PerMinute=TotalAmount/60.00;
		double Minute_15= PerMinute*15;
		return Math.round((Minute_15*multitime)*100.00)/100.00;
	...
	public class OTType {
	    ...
	    public OTType(String otType, String otId) {
	        this.otType = otType;
	        this.otId = otId;
	    }
	    ...
	    public String toString() {
	        return otType; // This is what will be displayed in the oTypeCB
	    }
	}
}