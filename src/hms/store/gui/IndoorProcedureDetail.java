...
public class IndoorProcedureDetail extends JDialog {
	...
	String itemIDSTR, itemNameSTR, itemDescSTR, taxTypeSTR, taxValueSTR, expiryDateSTR = "", issuedDateSTR = "",
			dueDateSTR = "", previouseStock = "", departmentName = "", departmentID = "", itemtypeSTR = "";
	double qtyIssued = 0, afterIssued = 0, itemValue, finalTaxValue = 0, finalDiscountValue = 0;
	int getselectedIndex = 0, removeSelectedIndex = 0;
	private String itemBatchNameSTR, item_ID = "";
	private String batchIDSTR;

	int quantity = 0, quantityTEMP = 0;
	...
	private JTextField issuedBtchQty;
	private JTextField batchQty;
	private JLabel Costlbl;

	public static void main(String arg[]) {
		new IndoorProcedureDetail("41530", "00000220234783", "Fistula");
	}

	public IndoorProcedureDetail(final String ipdidSTR, String patient_id, String type) {
		setIconImage(
				Toolkit.getDefaultToolkit().getImage(IndoorProcedureDetail.class.getResource("/icons/rotaryLogo.png")));
		...
		itemNameCB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				qtyInHandET.setText("");
				priceET.setText("");
				getItemStock(itemIDSTR);  //Arun: get common stock of item
				getItemDetail(itemIDSTR); //Arun: get full detail of item
		...
		panel_5.add(btnAdd);
		btnAdd.setIcon(new ImageIcon(IndoorProcedureDetail.class.getResource("/icons/plus_button.png")));
		btnAdd.setFont(new Font("Tahoma", Font.PLAIN, 12));

		btnRemove = new JButton("Remove");
		btnRemove.setBounds(359, 230, 125, 33);
		panel_5.add(btnRemove);
		btnRemove.setIcon(new ImageIcon(IndoorProcedureDetail.class.getResource("/icons/exit.png")));
		...
		scrollPane.setBounds(10, 262, 489, 251);
		...
		addTestTable_1.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				removeSelectedIndex = 0;
				if (e.getClickCount() == 1) {
					JTable target = (JTable) e.getSource();
					final int row = target.getSelectedRow();
					int column = target.getSelectedColumn();
					// do some action
					removeSelectedIndex = Integer.parseInt(addTestTable_1.getValueAt(row, 6) + "");
				}
			}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				itemIDV.remove(removeSelectedIndex);
				itemNameV.remove(removeSelectedIndex);
				itemDescV.remove(removeSelectedIndex);
				issuedQtyV.remove(removeSelectedIndex);
				itemPriceV.remove(removeSelectedIndex);
				totalPriceV.remove(removeSelectedIndex);
				ProcedureV.remove(removeSelectedIndex);
				ItemTypeRiskV.remove(removeSelectedIndex);
				itemMRPV.remove(removeSelectedIndex);
				itemMeasV.remove(removeSelectedIndex);
				batchNameV.remove(removeSelectedIndex);
				batchIDV.remove(removeSelectedIndex);
				itemEXPV.remove(removeSelectedIndex);
				loadDataToTable();
			}
		...
		scrollPane_2.setBounds(10, 24, 489, 179);
		...
		table_1.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getClickCount() == 1) {
					...
					quantityTEMP = (int) Double.parseDouble(table_1.getValueAt(row, 3).toString());
					getselectedIndex = Integer.parseInt(table_1.getValueAt(row, 5).toString());
					removeSelectedIndex = getselectedIndex;
					item_ID = id;
					itemIDSTR = id;
					getItemBatchName(id);
		...
		batchNameCB.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (batchNameCB.getSelectedIndex() > 0) {
					// System.out.print("batch id "+ batchIDSTR+" item id "+itemIDSTR);
					getItemBatch(batchIDSTR, itemIDSTR);
					batchQty.setText(quantity + "");
					if (quantityTEMP <= quantity) {
						issuedBtchQty.setEditable(false);
						issuedBtchQty.setText(quantityTEMP + "");

					} else {
						issuedBtchQty.setEditable(true);
					}

				}
			}
		});
		batchNameCB.setBounds(12, 213, 151, 20);
		panel_2.add(batchNameCB);

		issuedBtchQty = new JTextField();
		issuedBtchQty.setBounds(318, 215, 88, 19);
		panel_2.add(issuedBtchQty);
		issuedBtchQty.setColumns(10);
		issuedBtchQty.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
				char vChar = e.getKeyChar();
				if (!(Character.isDigit(vChar) || (vChar == KeyEvent.VK_BACK_SPACE) || (vChar == KeyEvent.VK_DELETE))) {
		...
		issuedBtchQty.getDocument().addDocumentListener(new DocumentListener() {
			@Override
			public void insertUpdate(DocumentEvent e) {
				String str = issuedBtchQty.getText() + "";
				if (!str.equals("")) {
					itemValue();
				}
			}
			...
			public void actionPerformed(ActionEvent e) {
				if ((batchNameCB.getSelectedItem().equals("select Batch"))) {
					JOptionPane.showMessageDialog(null, "Please select batch!", "Input Error",
							JOptionPane.ERROR_MESSAGE);
					return;
				} else if (issuedBtchQty.getText().toString().equals("")) {
					JOptionPane.showMessageDialog(null, "Please enter quantity!", "Input Error",
							JOptionPane.ERROR_MESSAGE);
					return;
				} else if (Integer.parseInt(issuedBtchQty.getText().toString()) > Integer
						.parseInt(batchQty.getText().toString())) {
					JOptionPane.showMessageDialog(null, "Batch Quantity Exceeded! please enter valide quantity",
							"Input Error", JOptionPane.ERROR_MESSAGE);
					return;
				} else if (quantityTEMP < Integer.parseInt(issuedBtchQty.getText().toString())) {
					JOptionPane.showMessageDialog(null, "Quantity Exceeded! please enter valide quantity",
							"Input Error", JOptionPane.ERROR_MESSAGE);
					return;
				} else if (itemIDV.indexOf(item_ID) != -1 && batchIDV.indexOf(batchIDSTR) != -1) {
					JOptionPane.showMessageDialog(null, "this item is already entered", "Input Error",
							JOptionPane.ERROR_MESSAGE);
					return;
				}

				// Now we can add batch
				System.out.print(getselectedIndex + " index " + itemBatchNameSTR + "-" + batchIDSTR);

				if (Integer.parseInt(batchQty.getText().toString()) < quantityTEMP) {
					int remainingQty = quantityTEMP - Integer.parseInt(issuedBtchQty.getText());
					issuedQtyV.removeElementAt(getselectedIndex);
					issuedQtyV.add(getselectedIndex, remainingQty + "");
					itemIDV.add(itemIDV.get(getselectedIndex));
					itemNameV.addElement(itemNameV.get(getselectedIndex));
					itemDescV.addElement(itemDescV.get(getselectedIndex));
					issuedQtyV.add(issuedBtchQty.getText() + "");
					itemPriceV.addElement(itemPriceV.get(getselectedIndex));
					totalPriceV.addElement(totalPriceV.get(getselectedIndex));
					ProcedureV.addElement(ProcedureV.get(getselectedIndex));
					ItemTypeRiskV.addElement(ItemTypeRiskV.get(getselectedIndex));
					itemMRPV.addElement(itemMRPV.get(getselectedIndex));
					itemMeasV.addElement(itemMeasV.get(getselectedIndex));
					batchNameV.addElement(itemBatchNameSTR);
					batchIDV.addElement(batchIDSTR);
					itemEXPV.addElement(itemEXPV.get(getselectedIndex));

				} else {
					batchNameV.removeElementAt(getselectedIndex);
					batchIDV.removeElementAt(getselectedIndex);
					totalPriceV.removeElementAt(getselectedIndex);

					totalPriceV.add(getselectedIndex, Costlbl.getText() + "");
					batchNameV.add(getselectedIndex, itemBatchNameSTR);
					batchIDV.add(getselectedIndex, batchIDSTR);
				}
		...
		btnNewButton_1.setBounds(418, 215, 68, 19);
		panel_2.add(btnNewButton_1);

		JLabel lblEnterQty_1 = new JLabel("Enter Qty ");
		lblEnterQty_1.setBounds(328, 235, 77, 15);
		panel_2.add(lblEnterQty_1);

		batchQty = new JTextField();
		batchQty.setText("");
		batchQty.setEditable(false);
		batchQty.setColumns(10);
		batchQty.setBounds(202, 215, 88, 19);
		panel_2.add(batchQty);

		JLabel lblEnterQty_1_1 = new JLabel("batch Stock");
		lblEnterQty_1_1.setBounds(202, 235, 87, 15);
		panel_2.add(lblEnterQty_1_1);

		JLabel lblEnterQty_1_2 = new JLabel("Cost :");
		lblEnterQty_1_2.setBounds(12, 235, 48, 25);
		panel_2.add(lblEnterQty_1_2);

		Costlbl = new JLabel("");
		Costlbl.setBounds(66, 235, 97, 25);
		panel_2.add(Costlbl);

		JButton btnNewButton_3 = new JButton("Save");
		btnNewButton_3.setIcon(new ImageIcon(IndoorProcedureDetail.class.getResource("/icons/SAVE.GIF")));
		btnNewButton_3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (patientNameTB.getText().equals("")) {
					...
					JOptionPane.showMessageDialog(null, "please provide batches to all items", "Input Error",
							JOptionPane.ERROR_MESSAGE);
					return;
					...
					if (flag == 1) {
						JOptionPane.showMessageDialog(null,
								result + "!Please Maintain Stock of this Items Or you can delete these from request",
								"Stock Less", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
					...
					for (int i = 0; i < itemIDV.size(); i++) {
						...
						datanew[4] = "" + DateFormatChange.StringToMysqlDate(new Date());
						...
						datanew[11] = db.retrieveItemSubCategory(itemIDV.get(i));
						...
						datanew[18] = batchIDV.get(i).toLowerCase().equals("n/a") ? "0" : batchIDV.get(i);
						if (!batchNameV.get(i).toLowerCase().equals("n/a")) {
							String str = batchNameV.get(i);
						...
						try {
							...
							if (!batchIDV.get(i).toLowerCase().equals("n/a")
									|| !batchNameV.get(i).toLowerCase().contains("n/a")) {
					...
					try {
						storePillsRegisterDBConnection.updateProcedureStatus(ipd_id);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					...
					try {
						new PillsSlippdf(ipdNoTB.getText(), itemNameV, issuedQtyV, totalPriceV, finalTotalValue + "",
								index, p_name, p_id, batchNameV, itemEXPV);
					} catch (Exception e) {
						// TODO: handle exception
					}
					dispose();
		...
		btnNewButton_4.setIcon(new ImageIcon(IndoorProcedureDetail.class.getResource("/icons/close_button.png")));
		...
		label_8.setIcon(new ImageIcon(IndoorProcedureDetail.class.getResource("/icons/graphics-hospitals-575145.gif")));
	...
	public void pillsEnteryData(String ipd_id) {
		...
		ResultSet resultSet = issuedItemsDBConnection.retrieveAllProcedurePillsDetail(ipd_id);
	...
	private void loadDataToTable() {
		...
		for (int i = 0; i < itemIDV.size(); i++) {
			if (checkflag(itemIDV.get(i)) && batchIDV.get(i).equals("n/a")) {
				c1++;
			} else {
				c2++;
			}
		...
		double total = 0;
		int a = 0, b = 0;
		ObjectArray_ListOfexamsSpecs = new Object[c2][10];
		ObjectArray_ListOfexamsSpecs1 = new Object[c1][10];

		for (int i = 0; i < itemIDV.size(); i++) {

			if (checkflag(itemIDV.get(i)) && batchIDV.get(i).equals("n/a")) {
				ObjectArray_ListOfexamsSpecs1[a][0] = itemIDV.get(i);
				ObjectArray_ListOfexamsSpecs1[a][1] = itemNameV.get(i);
				ObjectArray_ListOfexamsSpecs1[a][2] = itemDescV.get(i);
				ObjectArray_ListOfexamsSpecs1[a][3] = issuedQtyV.get(i);
				ObjectArray_ListOfexamsSpecs1[a][4] = totalPriceV.get(i);
				ObjectArray_ListOfexamsSpecs1[a][5] = i;
				a++;
				F = 1;
			} else {
				ObjectArray_ListOfexamsSpecs[b][0] = itemIDV.get(i);
				ObjectArray_ListOfexamsSpecs[b][1] = itemNameV.get(i);
				ObjectArray_ListOfexamsSpecs[b][2] = itemDescV.get(i);
				ObjectArray_ListOfexamsSpecs[b][3] = issuedQtyV.get(i);
				ObjectArray_ListOfexamsSpecs[b][4] = totalPriceV.get(i);
				ObjectArray_ListOfexamsSpecs[b][5] = batchNameV.get(i);
				ObjectArray_ListOfexamsSpecs[b][6] = i;
				b++;
			}
			total = total + Double.parseDouble(totalPriceV.get(i));
		}
		TableModel model1 = new EditableTableModel2(
				new String[] { "Item ID", "Item Name", "Item Desc.", "Issued Qty.", "Cost", "Item Batch", "index" },
				ObjectArray_ListOfexamsSpecs);
		TableModel model = new EditableTableModel2(
				new String[] { "Item ID", "Item Name", "Item Desc.", "Issued Qty.", "Cost", "index" },
				ObjectArray_ListOfexamsSpecs1);
	...
	private boolean checkflag(String index) {
		// TODO Auto-generated method stub

		ItemsDBConnection itemsDBConnection = new ItemsDBConnection();
		ResultSet resultSet = itemsDBConnection.CheckBatchReqd(index);
		try {
			while (resultSet.next()) {
	...
	public void populateExpensesTable(String ipd_id) {
		...
		try {
			...
			//Arun: you can use again resultSet by using beforefirst fun...
			rs.beforeFirst(); 
			...
			for (int i = 0; i < NumberOfRows; i++) {
				totalChargesIPD = totalChargesIPD + Double.parseDouble(Rows_Object_Array[i][1].toString());
			}
	...
	public void getItemBatch(String index, String item_id) {
		BatchTrackingDBConnection batchTrackingDBConnection = new BatchTrackingDBConnection();
		GeneralDBConnection formula = new GeneralDBConnection();
		double value = Double.parseDouble(formula.retrieveFormula1());
		formula.closeConnection();
		...
		ResultSet rs = batchTrackingDBConnection.itemBatchDetail(index);
		...
		try {
			while (rs.next()) {
				...
				price = Double.parseDouble(rs.getObject(5).toString());
	...
	public void itemValue() {
		...
		if (InsuranceMrpReqd(insuranceTypeTB.getText())) {
			itemValue = Double.parseDouble(issuedBtchQty.getText() + "") * mrp;
		} else {
			itemValue = Double.parseDouble(issuedBtchQty.getText() + "") * price;
			itemValue = Math.round(itemValue * 100.000) / 100.000;
		}
		Costlbl.setText(itemValue + "");
