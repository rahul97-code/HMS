...
public class TestHistoryData extends JPanel {
	 ...
	 * Create the panel.
	 */
	public TestHistoryData(String labNumber) {
		setLayout(null);
		this.labNumber=labNumber;
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(20, 44, 316, 314);
		add(scrollPane);
		readFile();
		widal.add("O");
		widal.add("H");
...
//					}
		...
		scrollPane_2.setBounds(346, 44, 303, 157);
		add(scrollPane_2);
		...
		lblResults.setBounds(346, 17, 124, 23);
		add(lblResults);
		
		JButton btnRecorrect = new JButton("Recorrect");
		btnRecorrect.setFont(new Font("Tahoma", Font.BOLD, 15));
		btnRecorrect.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				updateTestData(selectedObject+"");
				populateTestHistoryTable(p_id);
			}
		});
		btnRecorrect.setBounds(51, 369, 214, 39);
		add(btnRecorrect);
		searchExamTB.getDocument().addDocumentListener(new DocumentListener() {
			...
			public void insertUpdate(DocumentEvent e) {
				...
				if (!str.equals("")) {
					populateTestHistoryTable(p_id, str);
				} else {
					populateTestHistoryTable(p_id);
				}
			}
	...
	public void populateTestHistoryTable(String patientID) {
		p_id = patientID;
		fileList.clear();
		list.setListData(fileList);
		progressBar.setValue(0);
		try {
			ExamDBConnection db = new ExamDBConnection();
			ResultSet rs = db.retrieveTestHistoryDataPatientIDRoom(patientID,labNumber);
			...
			rs = db.retrieveTestHistoryDataPatientIDRoom(patientID,labNumber);
	...
	public void populateTestHistoryTable(String patientID, String searchText) {
		fileList.clear();
		list.setListData(fileList);
		progressBar.setValue(0);
		try {
			ExamDBConnection db = new ExamDBConnection();
			ResultSet rs = db.retrieveTestHistoryDataPatientIDRoom(patientID,
					searchText,labNumber);
			...
			// System.out.println("rows : " + NumberOfRows);
			// System.out.println("columns : " + NumberOfColumns);
			rs = db.retrieveTestHistoryDataPatientIDRoom(patientID, searchText,labNumber);
	...
	public void readFile() {
		...
		try {
			...
			mainDir = data[1];
			System.out.println("rahul dddddd ddddd"+data[1]);
			open[0]=data[2];
			open[1]=data[3];
			open[2]=data[4];
			// Always close files.
