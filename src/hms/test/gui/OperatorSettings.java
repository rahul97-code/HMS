...
public class OperatorSettings extends JDialog {

	private final JPanel contentPanel = new JPanel();
	private JPasswordField oldPasswordTB;
	private JPasswordField newPasswordTB;
	private JPasswordField confirmPasswordTB;
	String previousPass,newPass,confPass;
	boolean oldBL=false,newBL=false,congBL=false;

	/**
	 * Create the dialog.
	 */
	public OperatorSettings(final String operatorID) {
		setTitle("Change Password");
		setIconImage(Toolkit.getDefaultToolkit().getImage(OperatorSettings.class.getResource("/icons/rotaryLogo.png")));
		...
		ResultSet rs=db.retrievePassword(operatorID);
		try {
			while (rs.next()) {
				...
				okButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent arg0) {
						...
						}else
						{
							OperatorDBConnection db=new OperatorDBConnection();
							try {
								db.updateDataPassword(operatorID, newPass);
							} catch (Exception e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
								db.closeConnection();
