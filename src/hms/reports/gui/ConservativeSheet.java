...
public class ConservativeSheet extends JDialog {
	...
	Vector<String> DOC_IPD_ID=new Vector<>();
	...
	Vector<String> DISCOUNT=new Vector<>();
	Vector<Integer> DOC_MINUTES=new Vector<>();
	...
	Vector<String> FINAL_DISCOUNT=new Vector<>();
	...
	public static void main(String[] arg) {
		new ConservativeSheet().setVisible(true);
	}

	/**
	 * Create the frame.
	 */
	public ConservativeSheet() {
		setResizable(false);
		setTitle("Conservative Sheet");
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				ConservativeSheet.class.getResource("/icons/rotaryLogo.png")));
		setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		setBounds(100, 100, 1144, 524);
...
//				}
		...
		btnSearch.setIcon(new ImageIcon(ConservativeSheet.class.getResource("/icons/search_hover.png")));
		...
		btnExcel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (fileChooser.showSaveDialog(ConservativeSheet.this) == JFileChooser.APPROVE_OPTION) {
		...
		btnExcel.setIcon(new ImageIcon(ConservativeSheet.class
		...
		btnNewButton_1.setIcon(new ImageIcon(ConservativeSheet.class
		...
		panel_1.setBounds(198, 450, 934, 32);
	...
	private void loadDataToTable() {
		System.out.println(Arrays.toString(FINAL_DISCOUNT.toArray()));
		int totalMinutes=0;
		...
		for (int i = 0; i < FINAL_IPD_ID.size(); i++) {
			...
			if(FINAL_IPD_ID.get(i).equals(ipd) && FINAL_DOC_NAME.get(i).equals(doc)) {
				for(int j=7;j<uniqueHeadersArray.length;j++) {
				...
				ObjectArray_ListOfexamsSpecs[i][6] = FINAL_DISCOUNT.get(i);
				for(int j=7;j<uniqueHeadersArray.length;j++) {
					if(FINAL_WARD.get(i).equals(uniqueHeadersArray[j])){
		...
		table.getColumnModel().getColumn(7).setPreferredWidth(165);
		table.getColumnModel().getColumn(7).setMinWidth(165);
		table.getColumnModel().getColumn(8).setPreferredWidth(165);
		table.getColumnModel().getColumn(8).setMinWidth(165);
	...
	public void getFinalData() {
		for(int b=0;b<BED_IPD.size();b++) {
			for(int d=0;d<DOC_IPD_ID.size();d++) {
				if ((BED_IPD.get(b).equals(DOC_IPD_ID.get(d))) && (BED_MINUTES.get(b)>0 && 0<DOC_MINUTES.get(d))) {
					if (BED_MINUTES.get(b)>DOC_MINUTES.get(d)) {
						FINAL_WARD.add(WARD.get(b));
						FINAL_IPD_ID.add(DOC_IPD_ID.get(d));
						FINAL_DOC_NAME.add(DOC_NAME.get(d));
						FINAL_P_NAME.add(P_NAME.get(d));
						FINAL_P_ID.add(P_ID.get(d));
						FINAL_DOA.add(DOA.get(d));
						FINAL_INSURANCE.add(INSURANCE.get(d));
						FINAL_DISCOUNT.add(DISCOUNT.get(d));
	...
	public void getAllDoctorsData() {
		...
		ResultSet rs=db.getAllDocOfIpd(dateFrom, dateTo);
		try {
			while(rs.next()) {
				CHANGE_ID.add(rs.getString(1));
				DOC_IPD_ID.add(rs.getString(2));
				DOC_NAME.add(rs.getString(3));
				P_NAME.add(rs.getString(4));
				P_ID.add(rs.getString(5));
				DOA.add(rs.getString(6));
				DOC_MINUTES.add(rs.getInt(7));
				INSURANCE.add(rs.getString(8));
				DISCOUNT.add(rs.getString(9));
			}
	...
	public void getAllBedsData() {
		...
		uniqueWardSet.add("CONSULTANT");
		uniqueWardSet.add("DOA");
		uniqueWardSet.add("DISCOUNT");

		DoctorDBConnection db=new DoctorDBConnection(); 
		ResultSet rs=db.getAllPateintBeds(getAllIpd());
		try {
			while(rs.next()) {
				BED_IPD.add(rs.getString(1));
				if(!uniqueWardSet.contains(rs.getString(3))) {
					uniqueWardSet.add(rs.getString(3));
				}
				WARD.add(rs.getString(3));
				WARD_CAT.add(rs.getString(3));
				BED_DATE.add(rs.getString(4));
				BED_MINUTES.add(rs.getInt(5));					
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public String getAllIpd() {
		...
		for (int i=0;i<DOC_IPD_ID.size();i++)
		{
			if (DOC_IPD_ID.size() - 1 > i) {
				sb.append(DOC_IPD_ID.get(i)).append(",");
			} else {
				sb.append(DOC_IPD_ID.get(i));
			} 
		}
		return sb+"";
	}
