...
public class EditOPDtype extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public EditOPDtype(String opdid) {
		setResizable(false);
		setTitle("Edit OPD Type");
		setIconImage(Toolkit.getDefaultToolkit().getImage(EditOPDtype.class.getResource("/icons/rotaryLogo.png")));
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						{
							JOptionPane.showMessageDialog(null, "Please enter Charges 1",
									"Input Error", JOptionPane.ERROR_MESSAGE);
						}else{
							...
							try {
								priceMasterDBConnection.updateData(data);
							} catch (Exception e1) {
								// TODO Auto-generated catch block
								e1.printStackTrace();
							}
							priceMasterDBConnection.closeConnection();
							JOptionPane.showMessageDialog(null, "Edit OPD Type Successfully",
									"OPD Type edit", JOptionPane.INFORMATION_MESSAGE);
							dispose();
						}
					}
				});
	...
	public void getData(String opdID) {
		
		PriceMasterDBConnection dbConnection = new PriceMasterDBConnection();
		ResultSet resultSet = dbConnection.retrieveDataWithID(opdID);
		try {
			while (resultSet.next()) {
				nameTB.setText(resultSet.getObject(2).toString());
				charges1TB.setText(resultSet.getObject(3).toString());
				charges2TB.setText(resultSet.getObject(4).toString());
				charges3TB.setText(resultSet.getObject(5).toString());
				charges4TB.setText(resultSet.getObject(6).toString());
				
				selectInsuranceCB.setSelectedItem(resultSet.getObject(7).toString());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
