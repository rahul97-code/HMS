...
public class DoctorSettings extends JDialog {
	...
	public static void main(String[] args) {
		try {
			DoctorSettings dialog = new DoctorSettings("sukhpal");
	 ...
	 * Create the dialog.
	 */
	public DoctorSettings(final String doctorID) {
		setTitle("Change Password");
		setIconImage(Toolkit.getDefaultToolkit().getImage(DoctorSettings.class.getResource("/icons/rotaryLogo.png")));
		...
		ResultSet rs=db.retrievePassword(doctorID);
		try {
			while (rs.next()) {
				...
				okButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent arg0) {
						...
						}else
						{
							DoctorDBConnection db=new DoctorDBConnection();
							try {
								db.updateDataPassword(doctorID, newPass);
							} catch (Exception e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
								db.closeConnection();
